...
**Claim.** In high dimension, unbiasedness can waste noise length; shrinkage can reduce risk by spending a little bias.  
**Scope.** Classical James--Stein facts; simulations and geometry for intuition.
...
Classical statistics often treats unbiasedness as a cleanliness condition. James--Stein shows why that is too narrow: under squared-error loss, the relevant object is risk, and risk contains both bias and variance.

Bias helps here only because it buys a larger reduction in variance.
...
This formula is the whole bias--variance bargain. The first term is variance. It decreases when $a$ decreases. The second term is squared bias. It increases when $a$ moves away from one.
...
The theorem is different, but the geometry is similar:
...
This section is optional. It gives an asymptotic reading of the same shrinkage target phenomenon. 
...
Unbiasedness is a clean property. It is not the same as optimality.

In the normal-means problem, \(X\) is unbiased and has constant risk \(d\). But for \(d\ge3\), James--Stein shrinkage improves on it uniformly under total squared-error loss.

The price is bias. The reward is a larger reduction in variance.

The geometric reason is that high-dimensional noise has length. The usual estimator treats the whole observed vector as signal. Shrinkage refuses to do that.

Bias is useful only when it is doing identifiable work: moving an estimator away from a region where variance, boundary behavior, or unsupported directions dominate.
