...
**Claim.** Exact LSAP matching is exact for empirical measures, not automatically for population transport.  

**Scope.** Standard finite-assignment facts; examples about missing tail mass and out-of-sample interpretation.
...
> LSAP solves the finite matching problem exactly. It does not, by itself, turn two samples into a population transport rule.
...
> the empirical problem can be solved exactly while the population feature of interest is missing. This is not a numerical failure of assignment. It is a sampling failure hidden by an exact combinatorial solution.
...
LSAP returns a permutation $\sigma_n$. If the empirical assignment were literally the population map evaluated on the sample, then
...
The assignment certifies an exact coupling of two empirical measures. It does not, without an extension rule and sampling assumptions, certify a population transport map.

The gap has three sources. First, the assignment is defined only on the observed source atoms; for a continuous source distribution, this set has population measure zero. Second, rare target regions may be absent from the empirical target sample, so exact matching can still miss them entirely. Third, empirical OT is an estimator: exact finite optimization does not remove sampling error.

Thus the relevant question is not whether LSAP solved the finite assignment problem. It did. The question is whether the empirical coupling contains enough information to support the population transport claim being made.

When it does not, exact matching is the wrong comfort.
...
The solver literature is not the target here. Hungarian, Jonker–Volgenant, auction, and related methods solve their finite problems exactly. The issue begins after that: an exactly solved sampled problem can still omit the part of the population one cares about.
...
This section is optional; it explains what the LSAP coupling can produce under squared Euclidean cost. The empirical LSAP solution defines the coupling
