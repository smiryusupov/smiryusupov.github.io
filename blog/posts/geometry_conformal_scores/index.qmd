...
Here $\gamma=0$ keeps the score fully local, while $\gamma=1$ moves it back toward a global residual scale. The tuning parameter is a trust parameter: how much estimated local geometry should be allowed into the score?
...
Efficiency depends on whether the score ranks candidate labels according to the actual conditional structure of $Y$ given $X$. A well-matched score can shorten the conformal set; a poorly matched one can preserve coverage while wasting volume.
...
The coverage statements used above are the standard split-conformal ones. The new part is empirical and conceptual: different score geometries induce different set shapes and efficiencies.
