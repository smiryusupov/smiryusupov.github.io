...
**Claim.** A correct local Fisher metric does not guarantee selection of the globally desirable basin.  
**Scope.** Standard Fisher-geometry facts; small non-convex diagnostics comparing local preconditioners.
...
The note studies a failure mode of a local metric in a global problem. Even when the Fisher matrix is the correct infinitesimal geometry of the statistical model, it need not guide the trajectory toward the validation basin one would choose after seeing the whole landscape.
...
This empirical basin label is stricter than final training loss. A run may reduce the objective and still settle in the wrong region of the validation landscape.
...
The easy starts show that the task is not broken. All methods reach the good basin when the start is favorable.

The hard starts show the opposite extreme. In this run, no method reaches the good basin from the hard starts. These starts are useful as stress tests, but they should not dominate the interpretation.

The medium starts are the diagnostic regime. The good basin is reachable, but basin selection depends on the optimizer. In this run, exact Fisher NGD reaches the good basin on all medium starts. AdamW, diagonal exact Fisher, and SGD with momentum reach it on about half of the medium starts. Empirical Fisher NGD does not reach it from the medium starts.

This should not be read as a general benchmark victory for exact Fisher. The point is narrower: the local metric induces a trajectory, and the success of that trajectory depends on the initialization regime.
...
The pooled plot is useful only as a summary. It combines three qualitatively different regimes: easy starts where all methods succeed, hard starts where all methods fail, and medium starts where the methods actually differ.

For the question in this note, the medium regime carries most of the evidence. The pooled rate should therefore be read after the regime split, not as a standalone optimizer ranking.
...
K-FAC belongs to the same local-metric family, but with a structured approximation to the Fisher rather than the exact Fisher. For a linear layer with input activations $a$ and preactivation score gradients $\delta$, K-FAC approximates the Fisher block by
...
The experiment supports a narrower conclusion. Natural gradient and K-FAC are local metric methods: they can use the correct Fisher geometry and still remain agnostic about which distant basin is preferable.

Local correctness is only one part of the story. In a non-convex problem, the induced trajectory also has to reach the basin that matters for the task.
