...
**Claim.** Wasserstein loss is only as meaningful as the ground geometry supplied to it.  

**Scope.** Standard Gaussian Wasserstein and Fisher--Rao calculations; illustrative diagnostics.
...
Wasserstein distance answers the question posed by its ground geometry. Change the geometry, and the loss changes its meaning.
...
For some tasks, absolute displacement is the right unit: one meter, one dollar, or one pixel may have the same cost regardless of predictive uncertainty. For other tasks, the relevant error is relative to the distribution’s scale. In those cases, raw Wasserstein loss can measure the wrong discrepancy.
...
The ambiguity is interpretive: the numerical value of a Wasserstein loss inherits the ground geometry chosen for the sample space.
